...
// Name: Bloon() - Default Constructor
  ...
  ostream& operator<<(ostream& output, Bloon& bloonPtr){
    // returns the bloon color and health
    output << "Bloon: " << bloonPtr.GetColor();
    output << " Health: " << bloonPtr.GetHealth();
    return output;
  }